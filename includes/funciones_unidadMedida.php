...
function listarUnidadMedida()
	...
	$sql = " SELECT base.codprod as codProd, base.codumed as codUMed, base.valorflete as valorFlete, ";
	$sql.= " detalle.desprod as descripcionProducto, umed.desumed as descripcionMedida ";
	$sql.= " FROM ".$bd['softland'].".[iw_tprod] detalle";
	$sql.= " INNER JOIN  ".$bd['dsparam'].".[DS_FletesUmed] base ON detalle.Codprod = base.codprod ";
	...
	if($num_rows > 0)
		...
		<tr>
			...
			<th>Unidad Medida</th>
			<th nowrap="nowrap">Valor Flete</th>
		...
		while($row = sqlsrv_fetch_array($rec))
			...
			<input type="hidden" id="valorFlete_'.$i.'" value="'.$row['valorFlete'].'">
			<td align="right" >'.substr(formato_precio($row['valorFlete']),0,-1).'</td>			
